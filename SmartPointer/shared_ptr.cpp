...
int main(){
	...
	/**
	 * 注意1: shared_ptr 不能将一个原始指针直接赋值给一个智能指针来初始化
	 * shared_ptr<int>p4 = new int(600); // 编译报错，不允许直接赋值
	 */
	
	/**
	 * 注意2: shared_ptr 使用 reset 方法时
	 * 情况1: 对于未初始化的空指针，可以使用 reset 初始化
	 * 情况2: 当智能指针中有值时，使用 reset 会使引用计数减 1。
	 * 情况3: 可以使用 reset 让当前智能指针释放引用对象堆内存空间的所有权，
	 *       导致引用计数降低，不会导致堆内存的释放，只有当引用计数归零时，
	 *       shared_ptr 释放所占有的堆内存空间
	 */
	...
	// shared_ptr 管理数组时，需要指定删除器，因为 shared_ptr 默认删除器不支持数组对象。
	shared_ptr<int> ptr_arra(new int[3], [](int* p){ delete[] p; });
    ...
    /**
     * std::unique_ptr 管理动态数组 new T[size]
     */
